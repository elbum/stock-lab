...
class MongoDBHandlerTestCase(unittest.TestCase):
    ...
    def test_insert_items(self):
        ...
        _ids = self.mongodb.insert_items(docs, "stocklab_test", "corp_info")
        assert _ids
        print(_ids)

    def test_find_item(self):
        print(inspect.stack()[0][3])
        doc = self.mongodb.find_item(
            {"related": "LG"}, "stocklab_test", "corp_info")
        pprint(doc)

    def test_find_items(self):
        print(inspect.stack()[0][3])
        cursor = self.mongodb.find_items(
            {"tags.1": "red"}, "stocklab_test", "corp_info")
        assert cursor
        for doc in cursor:
            pprint(doc)
    ...
    def test_delete_items(self):
        print(inspect.stack()[0][3])
        result = self.mongodb.delete_items(
            {"related": "SamSung"}, "stocklab_test", "corp_info")
        assert result
    ...
    def test_update_items(self):
        ...
        result = self.mongodb.update_items(
            {"item": "LG Telecom"}, {"$set": {"qty": 300}}, "stocklab_test", "corp_info")
        assert result
    ...
    def test_aggregate(self):
        ...
        pipeline = [
            {"$match": {"tags.1": "red"}},
            {"$group": {"_id": "$related", "sum_val": {"$sum": "$qty"}}}
        ]
    ...
    def test_text_search(self):
        ...
        print("index create result = ", index_result)
        result = self.mongodb.text_search(
            "blank", "stocklab_test", "corp_info")
        assert result
        for item in result:
            pprint(item)
