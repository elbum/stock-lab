...
def collect_stock_info():
    ...
    collect_list = mongodb.find_items(
        {"날짜": today}, "stocklab", "price_info").distinct("code")
    ...
    for code in target_code:
        ...
        result_short = ebest.get_short_trend_by_code(
            code, sdate=today, edate=today)
        if len(result_short) > 0:
            mongodb.insert_items(result_short, "stocklab", "shortinfo")

        result_agent = ebest.get_agent_trend_by_code(
            code, fromdt=today, todt=today)
...
if __name__ == "__main__":
    ...
    scheduler.add_job(func=run_process_collect_code_list, trigger="cron",
                      day_of_week="mon-fri", hour="22", minute="37", id="1")
