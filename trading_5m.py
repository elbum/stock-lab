...
def check_buy_completed_order(code):
    ...
    for buy_completed_order in buy_completed_order_list:
        ...
        sell_order = ebest_demo.order_stock(
            code, "2", str(sell_price), "1", "00")

        print("order_stock", sell_order)
        mongo.update_item({"매수주문.주문번호": buy_order_no},
                          {"$set": {"매도주문": sell_order[0], "status": "sell_ordered"}}, "stocklab_demo", "order")


def check_buy_order(code):
    # 매수주문 완료 체크

    order_list = list(mongo.find_items(
        {"$and": [{"code": code}, {"status": "buy_ordered"}]}, "stocklab_demo", "order"))
    for order in order_list:
        ...
        if order_cnt == result_cnt:
            mongo.update_item({"매수주문.주문번호": order_no},
                              {"$set": {"매수완료": check_result, "status": "buy_completed"}}, "stocklab_demo", "order")
...
def check_sell_order(code):
    sell_order_list = list(mongo.find_items(
        {"$and": [{"code": code}, {"status": "sell_ordered"}]}, "stocklab_demo", "order"))
    ...
    for order in sell_order_list:
        ...
        if order_cnt == result_cnt:
            mongo.update_item({"매도주문.주문번호": order_no},
                              {"$set": {"매도완료": check_result, "status": "sell_completed"}}, "stocklab_demo", "order")
...
def trading_scenario(code_list):
    for code in code_list:
        ...
        if buy_order_cnt == 0:
            ...
            order_doc = order[0]
            mongo.insert_item({"매수주문": order_doc, "code": code,
                               "status": "buy_ordered"}, "stocklab_demo", "order")
...
if __name__ == "__main__":
    ...
    print("today", today)
    scheduler.add_job(func=run_process_trading_scenario, trigger="interval",
                      minutes=1, id="demo", kwargs={"code_list": code_list})
    scheduler.start()
    while True:
